...
const slots_routes_1 = require("../slots/slots.routes"); // Mocks
class RecommendationService {
    ...
    async calculateRecommendation(inputs) {
        const centers = await (0, centers_service_1.getAllCenters)(inputs.farmerId);
        ...
        const scoredOptions = eligibleCenters.map((center) => {
            ...
            const slot = slots_routes_1.DEMO_SLOTS.find((s) => s.center_id === center.id && s.status === 'AVAILABLE') || slots_routes_1.DEMO_SLOTS[0];
            // Normalize Queue [0, 1]
            ...
            const totalScore = (0.35 * normalizedQueue) +
