...
const MOCK_USERS_BY_ID = new Map(Array.from(MOCK_USERS.values()).map((u) => [u.id, u]));
// ─── Service ──────────────────────────────────────────────────────────────────
/**
...
async function sendOtp(mobile) {
    ...
    logger_1.logger.info(`[Auth] OTP simulated for ${mobile}. Use "${DEMO_OTP}" to verify.`);
    return { success: true, data: { mobile, otpSent: true, expiresInSec: 300, demoOtp: DEMO_OTP } };
}
/**
...
async function verifyOtp(mobile, otp) {
    ...
    // ── Mock mode ─────────────────────────────────────────────────────────────
    if (!(0, supabase_1.isDatabaseConfigured)()) {
...
async function registerFarmer(input) {
    ...
    if (!(0, supabase_1.isDatabaseConfigured)()) {
        ...
        logger_1.logger.info(`[Auth] Mock farmer registered: ${farmerIdCode} (${mobile})`);
    ...
    if (userError || !newUser) {
        logger_1.logger.error('[Auth] Failed to create user:', userError);
        return { success: false, error: 'Registration failed. Please try again.', errorCode: 'DB_INSERT_ERROR' };
    }
    // Insert into farmer_profiles
    const { error: profileError } = await supabase_1.supabase.from('farmer_profiles').insert({
        user_id: newUser.id,
    ...
    if (profileError) {
        logger_1.logger.error('[Auth] Failed to create farmer profile (non-fatal):', profileError);
    }
    logger_1.logger.info(`[Auth] Farmer registered: ${farmerIdCode} (${mobile})`);
...
async function updateUserProfile(userId, updates) {
    ...
    const { data, error } = await supabase_1.supabase
        ...
        .single();
    if (error || !data) {
        logger_1.logger.error('[Auth] Failed to update user profile:', error);
