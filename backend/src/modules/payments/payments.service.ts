...
export async function generatePaymentForProcurement(procurement: Procurement): Promise<PaymentRecord> {
  // Check if payment already exists
  const existing = DEMO_PAYMENTS.find(p => p.procurement_id === procurement.id);
  ...
  await notificationService.dispatch(
    ...
    `Simulated DBT payment of ₹${net.toLocaleString()} for ${qty} Qtl ${procurement.crop} has been initiated.`,
    'INFO'
  );
  ...
  setTimeout(async () => {
    ...
    await notificationService.dispatch(
      procurement.farmer_id,
      'Payment Credited',
      `Simulated DBT payment of ₹${net.toLocaleString()} has been credited to your registered account.`,
      'SUCCESS'
    );
  }, 10000);
...
export async function getMyPayments(farmerId: string): Promise<PaymentRecord[]> {
  return DEMO_PAYMENTS.filter(p => p.farmer_id === farmerId).sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
}
